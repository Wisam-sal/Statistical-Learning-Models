...
def model_analytics(y_test, y_pred, model_name):
    ...
    mse_ci = bootstrap((y_test, y_pred), 
                       lambda y, yhat: mean_squared_error(y, yhat), 
                       confidence_level=0.95, n_resamples=1000, method="basic").confidence_interval
    ...
    print("="*60)
    print(f"MSE: {mse:.4f} (95% CI: [{mse_ci.low:.4f}, {mse_ci.high:.4f}])")
    print(f"RMSE: {rmse:.4f}")
    print(f"MAE: {mae:.4f}")
    print(f"R^2: {r2:.4f}")
    print(f"Residual Mean: {np.mean(res):.4f}, Residual Std: {np.std(res):.4f}")
    print(f"Shapiro-Wilk Test p-value: {shapiro_p:.4f} (normality {'not rejected' if shapiro_p > 0.05 else 'rejected'})")
    print("="*60)